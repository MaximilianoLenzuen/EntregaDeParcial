    ...
    if(guardarIndice != -1){
        ...
        for(h=0;h<tamRaza;h++){
            if(auxRaza==razaPais[h].idRaza){
                strcpy(mascota[guardarIndice].raza,razaPais[h].raza);
            }
        }
...
int modificarMascotaConDuenio(eMascotas mascota[],int tamMascota,eCliente cliente[],int tamCliente, int estado){
    int i;
    int idModificar;
    int aux;
